...
export const Quiz: React.FC<QuizProps> = ({ onComplete }) => {
  ...
  // --- VIEW: QUIZ ---
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center p-6 md:p-12 relative overflow-hidden font-sans">
       ...
       <button 
          onClick={toggleMute} 
          className="absolute top-6 right-6 z-50 p-3 bg-white/50 backdrop-blur-sm rounded-full hover:bg-white transition-all text-stone-600 border border-stone-200 shadow-sm no-print"
      ...
      <div className={`w-full relative z-10 ${isZodiacQuestion ? 'max-w-4xl' : 'max-w-2xl'}`}>
        ...
        <div className={`transition-all duration-500 transform ease-in-out ${isTransitioning ? 'opacity-0 translate-y-8' : 'opacity-100 translate-y-0'}`}>
            ...
            {/* Question */}
            <h2 className="text-3xl md:text-5xl font-serif text-stone-900 leading-[1.1] mb-12 text-center md:text-left animate-fade-in">
              {currentQuestion.question}
            </h2>

            {/* Options */}
            <div className={isZodiacQuestion ? "grid grid-cols-2 md:grid-cols-3 gap-4" : "space-y-4"}>
              {currentQuestion.options.map((option, idx) => (
                <button
                  ...
                  className={`group relative bg-white border border-stone-100 hover:border-stone-900 shadow-sm hover:shadow-xl hover:shadow-stone-200/50 transition-all duration-300 rounded-lg overflow-hidden
                    ${isZodiacQuestion 
                      ? "p-6 flex flex-col items-center justify-center text-center aspect-[4/3]" 
                      : "w-full p-6 md:p-8 text-left flex items-center justify-between"
                    }`}
                ...
                >
                  {isZodiacQuestion ? (
                    // --- ZODIAC CARD LAYOUT ---
                    <>
                      <div className="text-4xl text-primary mb-3 font-serif group-hover:scale-110 transition-transform duration-500">
                        {option.symbol}
                      </div>
                      <span className="font-serif text-lg text-stone-900 mb-1 block">
                        {option.text}
                      </span>
                    ...
                    <>
                       {/* Text */}
                      <span className="font-reading text-lg md:text-xl text-stone-600 group-hover:text-stone-900 transition-colors duration-300 relative z-10">
                        {option.text}
                      </span>
                      
                      {/* Icon */}
                      <span className="text-stone-300 group-hover:text-stone-900 transform translate-x-8 group-hover:translate-x-0 opacity-0 group-hover:opacity-100 transition-all duration-300 relative z-10">
                        <ArrowRight size={20} />
                      </span>
                    </>
                  )}
